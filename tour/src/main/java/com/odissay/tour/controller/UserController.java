...
import com.odissay.tour.service.UserService;
...
public class UserController {
    ...
    @Operation(
            summary = "UPDATE USER",
            description = "Questo metodo consente ad un utente (autenticato) di aggiornare le proprie informazioni realtive a username, email, firstname e lastname.",
            tags = {"User"},
            responses = {
                    @ApiResponse(responseCode="200", description="Aggiornamento avvenuto con successo. E' necessario riloggarsi", content = @Content(schema = @Schema(implementation = UserDetailResponse.class))),
    ...
    @Operation(
            ...
            description = "Questo metodo restituisce data/ora dell'ultimo login dell'utente identificato dal proprio id.",
            tags = {"User"},
            responses = {
                    @ApiResponse(responseCode="200", description="Ultimo login resituito con successo.", content = @Content(schema = @Schema(implementation = String.class))),
                    @ApiResponse(responseCode="400", description="Il valore di userId non è quello atteso.", content = @Content(schema = @Schema(implementation = CustomErrorResponse.class))),
                    @ApiResponse(responseCode="404", description="L'utente cercato non si è mai loggato", content = @Content(schema = @Schema(implementation = CustomErrorResponse.class)))
            }
    )
